...
function docker::build() {
	local -r targets=($@)
	version::git_version
	local -r image_tag="${IMAGE_TAG:-"${GIT_VERSION}"}"
	local output_type="docker"
	if [[ $OUTPUT_TYPE == "registry" ]]; then
		output_type="registry"
	fi
	for target in "${targets[@]}"; do
		local image_name="${IMAGE_NAME:-"${REGISTRY}/${target}:${image_tag}"}"
		...
		docker buildx build --progress=plain --output=type=$output_type \
			...
			--build-arg ="${target}" \
			--tag "${image_name}" \
			--file "${ROOT_PATH}/${DOCKERFILE_PATH[${target}]}" \
			"${ROOT_PATH}/${CODE_PATH[$target]}"
		set +x
	done
}
